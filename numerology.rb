...
def birth_message(number)
  ...
  when 4
    puts "Your number is #{number}.\nThis is the worker. Practical, with a love of detail, Fours are trustworthy, hard-working, 
          and helpful. Ruled by Uranus. " 
  when 5 
    puts "Your number is #{number}.\nThis is the freedom lover. The number five is an intellectual vibration. These are idea
          people with a love of variety and the ability to adapt to most situations. Ruled by Mercury."       
  when 6
    puts "Your number is #{number}.\nOThis is the peace lover. The number six is a loving, stable, and harmonious vibration. 
          Ruled by Venus. "   
  when 7 
  ...
  when 8
    puts "Your number is #{number}.\nThis is the manager. Number Eight is a strong, successful, and material vibration. 
          Ruled by Saturn." 
  when 9
    puts "Your number is #{number}.\nThis is the teacher. Number Nine is a tolerant, somewhat impractical, and sympathetic 
...
birth_number = birthday_number(birthdate)
b_message = birth_message(birth_number)
